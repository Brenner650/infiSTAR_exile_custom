...
try 
{
					...
					{					
						if ((floor (random 100)) <= _spawnChance) then
						{
							...
							if !(_itemClassName in _spawnedItemClassNames) then
							{
								...
								_lootHolder setVariable ["ExileSpawnedAt", time + 300];
								switch (_cargoType) do
								{
									case 1: 	{ _lootHolder addMagazineCargoGlobal [_itemClassName, 1]; };
									case 3: {
										if!(owner _lootHolder isEqualTo 2)then{_lootHolder setOwner 2;};
										_lootHolder addBackpackCargo [_itemClassName, 1];
									};
									case 2: 	
									{ 
	...
	if (_notifyPlayer == 1) then
	{
		if (_spawnedLootForThisPlayer) then
		{
			...
			} forEach ([_playerPosition, _spawnRadius] call hotfix_getNearbyPlayers);
		};
	};
